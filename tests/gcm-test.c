/*
 *  This test adopts unit tests from mbed TLS GCM project (https://tls.mbed.org)
...
int main(void)
{
    ...
    for (j=0; j<3; j++)
    {
        ...
        for (i=0; i<MAX_TESTS; i++)
        {
            ...
            ret = gcm_crypt_and_tag(
                &ctx, GCM_ENCRYPT, pt_len[i],
                iv[iv_index[i]], iv_len[i],
                additional[add_index[i]], add_len[i],
                pt[pt_index[i]], buf, 16, tag_buf);

            if (ret ||
                memcmp(buf, ct[j*6+i], pt_len[i]) ||
                memcmp(tag_buf, tag[j*6+i], 16))
            {
                printf("failed\n");
                return 1;
            }
            ...
            ret = gcm_crypt_and_tag(
                &ctx, GCM_DECRYPT, pt_len[i],
                iv[iv_index[i]], iv_len[i],
                additional[add_index[i]], add_len[i],
                ct[j*6+i], buf, 16, tag_buf);

            if (ret ||
                memcmp(buf, pt[pt_index[i]], pt_len[i]) ||
                memcmp(tag_buf, tag[j*6+i], 16))
            {
                printf("failed\n");
                return 1;
